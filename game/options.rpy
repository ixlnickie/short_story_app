...
define gui.about = _p("""
    Вы — новый сотрудник в крупном банке "Финанс-Капитал". 
    Впереди — сложные задачи, выбор что делать дальше и... живые люди. 
    Каждое ваше решение: какой технологией заняться, как ответить на критику, кому помочь — определяет ваш профессиональный путь. 
    Сможете ли вы пройти путь от стажёра до ведущего специалиста, не сломавшись и не растеряв себя? Ваша история начинается с первого коммита.
""")
...
define config.save_directory = "luba-1765829313"
